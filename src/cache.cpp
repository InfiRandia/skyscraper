...
bool Cache::read()
{
  ...
  if (quickIdFile.open(QIODevice::ReadOnly))
  {
    ...
    while (!xml.atEnd())
    {
      ...
      if (!attribs.hasAttribute("filepath") ||
        !attribs.hasAttribute("timestamp") || !attribs.hasAttribute("id"))
      {
        continue;
      }
  ...
  if (cacheFile.open(QIODevice::ReadOnly))
  {
    ...
    while (!xml.atEnd())
    {
      ...
      {
        printf("Resource with cache id '%s' is missing 'type' attribute, "
          "skipping...\n",
      ...
      {
        printf("Resource with cache id '%s' is missing 'timestamp' attribute, "
          "skipping...\n",
          resource.cacheId.toStdString().c_str());
        continue;
      }
      ...
      if (resource.type == "cover" || resource.type == "screenshot" ||
        resource.type == "wheel" || resource.type == "marquee" || resource.type == "texture" ||
        resource.type == "video")
      {
        if (!QFileInfo::exists(cacheDir.absolutePath() + "/" +
          resource.value))
        {
  ...
  const QString& type)
{
  ...
  if (!command.isEmpty())
  {
    if (command == "new")
    {
      if (type != "title" && type != "platform" && type != "releasedate" &&
        type != "developer" && type != "publisher" && type != "players" &&
        type != "ages" && type != "genres" && type != "rating" &&
        type != "description")
      {
        printf(
          "Unknown resource type '%s', please specify any of the following: "
          "'title', 'platform', 'releasedate', 'developer', 'publisher', "
    ...
    {
      printf(
        "Unknown command '%s', please specify one of the following: 'new'.\n",
  ...
  printf("\033[1;33mEntering resource cache editing mode! This mode allows you "
    "to edit textual resources for your files. To add media resources use "
    "the 'import' scraping module instead.\nNote that you can provide one "
    "or more file names on command line to edit resources for just those "
    "specific files. You can also use the '--startat' and '--endat' "
    "command line options to narrow down the span of the roms you wish to "
    "edit. Otherwise Skyscraper will edit ALL files found in the input "
    "folder one by one.\033[0m\n\n");
  while (queue->hasEntry())
  {
    ...
    while (!doneEdit)
    {
      ...
      if (command.isEmpty())
      {
        printf("\033[1;34mWhat would you like to do?\033[0m (Press enter to "
          "continue to next rom in queue)\n");
        printf("\033[1;33ms\033[0m) Show current resource priorities for this "
          "rom\n");
        printf("\033[1;33mS\033[0m) Show all cached resources for this rom\n");
        printf("\033[1;33mn\033[0m) Create new prioritized resource for this "
          "rom\n");
        printf("\033[1;33md\033[0m) Remove specific resource connected to this "
          "rom\n");
        printf(
          "\033[1;33mD\033[0m) Remove ALL resources connected to this rom\n");
        printf("\033[1;33mm\033[0m) Remove ALL resources connected to this rom "
          "from a specific module\n");
        printf("\033[1;33mt\033[0m) Remove ALL resources connected to this rom "
          "of a specific type\n");
      ...
      else if (userInput == "n")
      {
        ...
        if (type.isEmpty())
        {
          ...
          printf("\033[1;33m4\033[0m) Publisher %s\n",
            ...
            .c_str());
          printf(
            "\033[1;33m5\033[0m) Number of players %s\n",
            QString((game.playersSrc.isEmpty() ? "(\033[1;31mmissing\033[0m)"
              : ""))
            .toStdString()
            .c_str());
          printf("\033[1;33m6\033[0m) Age rating %s\n",
            QString((game.agesSrc.isEmpty() ? "(\033[1;31mmissing\033[0m)"
              : ""))
            .toStdString()
            .c_str());
          printf("\033[1;33m7\033[0m) Genres %s\n",
            QString((game.tagsSrc.isEmpty() ? "(\033[1;31mmissing\033[0m)"
              : ""))
            .toStdString()
            .c_str());
          printf(
            "\033[1;33m8\033[0m) Game rating %s\n",
        ...
        else
        {
          ...
          if (typeInput == "0")
          {
            newRes.type = "title";
            printf(
              "\033[1;34mPlease enter title:\033[0m (Enter to cancel)\n> ");
            getline(std::cin, valueInput);
          }
          else if (typeInput == "1")
          {
            newRes.type = "platform";
            printf("\033[1;34mPlease enter platform:\033[0m (Enter to "
              "cancel)\n> ");
            getline(std::cin, valueInput);
          }
          else if (typeInput == "2")
          {
            ...
            printf("\033[1;34mPlease enter a release date in the format "
              "'yyyy-MM-dd':\033[0m (Enter to cancel)\n> ");
            getline(std::cin, valueInput);
          ...
          else if (typeInput == "3")
          {
            ...
            printf("\033[1;34mPlease enter developer:\033[0m (Enter to "
              "cancel)\n> ");
            getline(std::cin, valueInput);
          }
          else if (typeInput == "4")
          {
            ...
            printf("\033[1;34mPlease enter publisher:\033[0m (Enter to "
              "cancel)\n> ");
            getline(std::cin, valueInput);
          }
          else if (typeInput == "5")
          {
            newRes.type = "players";
            printf("\033[1;34mPlease enter highest number of players such as "
              "'4':\033[0m (Enter to cancel)\n> ");
            getline(std::cin, valueInput);
            expression = "^[0-9]{1,2}$";
          }
          else if (typeInput == "6")
          {
            newRes.type = "ages";
            printf(
              "\033[1;34mPlease enter lowest age this should be played at "
              "such as '10' which means 10+:\033[0m (Enter to cancel)\n> ");
            getline(std::cin, valueInput);
          ...
          else if (typeInput == "7")
          {
            newRes.type = "tags";
            printf(
              "\033[1;34mPlease enter comma-separated genres in the format "
              "'Platformer, Sidescrolling':\033[0m (Enter to cancel)\n> ");
            getline(std::cin, valueInput);
          }
          else if (typeInput == "8")
          {
            newRes.type = "rating";
            printf("\033[1;34mPlease enter game rating from 0.0 to 1.0:\033[0m "
              "(Enter to cancel)\n> ");
            getline(std::cin, valueInput);
          ...
          else if (typeInput == "9")
          {
            ...
            printf("\033[1;34mPlease enter game description. Type '\\n' for "
              "newlines:\033[0m (Enter to cancel)\n> ");
            getline(std::cin, valueInput);
          }
          else
          {
          ...
          if (valueInput == "")
          {
            ...
            QRegularExpression(expression).match(value).hasMatch())
          {
            newRes.value = value;
            bool updated = false;
            ...
            while (it.hasNext())
            {
              ...
              if (res.cacheId == newRes.cacheId && res.type == newRes.type &&
      ...
      else if (userInput == "d")
      {
        int b = 1;
        QList<int> resIds;
        printf("\033[1;34mWhich resource id would you like to remove?\033[0m "
          "(Enter to cancel)\n");
        ...
        else
        {
          ...
          {
            resources.removeAt(
              resIds.at(chosen - 1)); // -1 because lists start at 0
      ...
      else if (userInput == "D")
      {
        ...
        while (it.hasNext())
        {
          ...
          if (res.cacheId == cacheId)
          {
            printf("<<< Removed \033[1;33m%s\033[0m (%s) with value "
              "'\033[1;32m%s\033[0m'\n",
              res.type.toStdString().c_str(),
              res.source.toStdString().c_str(),
              res.value.toStdString().c_str());
      ...
      {
        printf("\033[1;34mResources from which module would you like to "
        ...
        else if (modules.contains(QString(typeInput.c_str())))
        {
          ...
          printf("<<< Removed %d resource(s) connected to rom from module "
            "'\033[1;32m%s\033[0m'\n\n",
            removed, typeInput.c_str());
        }
        else
        {
          printf("No resources from module '\033[1;32m%s\033[0m' found, "
            "cancelling...\n\n",
            typeInput.c_str());
        }
      }
      else if (userInput == "t")
      {
        ...
        else if (types.contains(QString(typeInput.c_str())))
        {
          ...
          printf("<<< Removed %d resource(s) connected to rom of type "
            "'\033[1;32m%s\033[0m'\n\n",
            removed, typeInput.c_str());
        }
        else
  ...
  while (it.hasNext())
  {
    ...
    if (remove)
    {
      if (res.type == "cover" || res.type == "screenshot" ||
        res.type == "wheel" || res.type == "marquee" || res.type == "texture" || res.type == "video")
      {
...
bool Cache::purgeAll(const bool unattend)
{
  if (!unattend)
  {
    printf("\033[1;31mWARNING! You are about to purge / remove ALL resources "
      "from the Skyscaper cache connected to the currently selected "
      "platform. THIS CANNOT BE UNDONE!\033[0m\n\n");
  ...
  {
    printf(
      "No resources for the current platform found in the resource cache.\n");
    return false;
  }
  else
  {
  ...
  const bool subdirs)
{
  ...
  if (filter.size() >= 2)
  {
    QDirIterator dirIt(inputFolder, filters, QDir::Files | QDir::NoDotAndDotDot,
...
void Cache::assembleReport(const Settings& config, const QString filter)
{
  ...
  {
    printf("Don't understand report option, please check '--cache help' for "
      "more info.\n");
    return;
  }
  ...
  {
    if (resType != "title" && resType != "platform" &&
      resType != "description" && resType != "publisher" &&
      resType != "developer" && resType != "ages" && resType != "players" &&
      resType != "tags" && resType != "rating" && resType != "releasedate" &&
      resType != "cover" && resType != "screenshot" && resType != "wheel" &&
      resType != "marquee" && resType != "texture" && resType != "video")
    {
      if (resType != "help")
      {
      ...
      printf(
        "  \033[1;32mall\033[0m: Creates reports for all resource types\n");
      printf("  \033[1;32mtextual\033[0m: Creates reports for all textual "
        "resource types\n");
      printf("  \033[1;32martwork\033[0m: Creates reports for all artwork "
        "related resource types excluding 'video'\n");
      printf("  \033[1;32mmedia\033[0m: Creates reports for all media resource "
        "types including 'video'\n");
      printf("  \033[1;32mtype1,type2,type3,...\033[0m: Creates reports for "
  ...
  if (!reportsDir.exists())
  {
    ...
    {
      printf("Couldn't create reports folder '%s'. Please check permissions "
        "then try again...\n",
  ...
  {
    printf("Length of cache id list mismatch the number of files, something is "
      "wrong! Please report this. Can't continue...\n");
    return;
  }
  ...
  for (const auto& resType : resTypeList)
  {
    QFile reportFile(reportsDir.absolutePath() + "/report-" + config.platform +
      "-missing_" + resType + "-" + dateTime + ".txt");
    printf("Report filename: '\033[1;32m%s\033[0m'\nAssembling report, please "
      "wait...",
    ...
    if (reportFile.open(QIODevice::WriteOnly))
    {
      ...
      reportFile.close();
      printf("\033[1;32m Done!\033[0m\n\033[1;33m%d file(s) is/are missing the "
        "'%s' resource.\033[0m\n\n",
  ...
  printf(
    ...
    "--fromfile <REPORTFILE>\033[0m' or the '\033[1;33m-s import\033[0m' "
    "module to add the missing resources. Check '\033[1;33m--help\033[0m' "
    "and '\033[1;33m--cache help\033[0m' for more information.\n\n");
}
  ...
  const int verbosity, const bool unattend)
{
  if (!unattend)
  {
    ...
    printf(
      ...
      "resources that don't match your current romset (files located at '%s' "
      "or any of its subdirectories matching the suffixes supported by the "
      "platform and any extension(s) you might have added manually). Please "
      "consider making a backup of your Skyscraper cache before performing "
      "this action. The cache for this platform is listed under 'Cache "
      "folder' further up and is usually located under "
      "'/home/USER/.skyscraper/' unless you've set it manually.\033[0m\n\n",
  ...
  int vacuumed = 0;
  {
    ...
    while (it.hasNext())
    {
      ...
      {
        if (res.type == "cover" || res.type == "screenshot" ||
          res.type == "wheel" || res.type == "marquee" || res.type == "texture" ||
          res.type == "video")
        {
          if (!QFile::remove(cacheDir.absolutePath() + "/" + res.value))
          {
  ...
  {
    printf(
      "All resources match a file in your romset. No resources vacuumed.\n");
    return false;
  }
  else
  {
...
void Cache::readPriorities()
{
  ...
  printf("Looking for optional '\033[1;33mpriorities.xml\033[0m' file in cache "
    "folder... ");
...
void Cache::validate()
{
  ...
  else
  {
    ...
    if (filesNoDelete != 0)
    {
      printf("%d files couldn't be deleted, please check file permissions and "
        "re-run with '--cache validate'.\n",
        filesNoDelete);
    }
    printf("\n");
  }
  ...
  for (const auto& mergeResource : mergeResources)
  {
        ...
        res.source == mergeResource.source)
      {
        if (overwrite)
        {
            ...
            res.type == "video")
          {
            ...
            {
              printf(
                "Couldn't remove media file '%s' for updating, skipping...\n",
                res.value.toStdString().c_str());
              continue;
            }
          }
    ...
    if (!resExists)
    {
      if (mergeResource.type == "cover" || mergeResource.type == "screenshot" ||
        mergeResource.type == "wheel" || mergeResource.type == "marquee" || mergeResource.type == "texture" ||
        mergeResource.type == "video")
      {
        cacheDir.mkpath(
          QFileInfo(cacheDir.absolutePath() + "/" + mergeResource.value)
          .absolutePath());
  ...
  const Settings& config, QString& output)
{
  ...
  if (notFound)
  {
          ...
          if (imageData->size() > resizedData.size())
          {
            if (config.verbosity >= 3)
            {
              printf("%s: '%d' > '%d', choosing resize for optimal result!\n",
                resource.type.toStdString().c_str(), imageData->size(),
                resizedData.size());
            }
    ...
    else if (resource.type == "video")
    {
      if (entry.videoData.size() <= config.videoSizeLimit)
      {
        ...
        if (f.open(QIODevice::WriteOnly))
        {
          ...
          if (!config.videoConvertCommand.isEmpty())
          {
            ...
            if (doVideoConvert(resource, cacheFile, cacheAbsolutePath, config,
              output))
            {
            ...
            {
              output.append("\033[1;31mFailed!\033[0m (set higher '--verbosity "
                "N' level for more info)");
              f.remove();
              okToAppend = false;
            }
          }
      ...
      {
        output.append("Video exceeds maximum size of " +
          QString::number(config.videoSizeLimit / 1024 / 1024) +
          " MB. Adjust this limit with the 'videoSizeLimit' "
          "variable in '/home/USER/.skyscraper/config.ini.'");
        okToAppend = false;
      }
    }
    ...
    {
      printf("\033[1;33mWarning! Couldn't add resource to cache. Have you run "
        "out of disk space?\n\033[0m");
    }
  }
}
