#!/bin/bash
{
    LATEST=`wget -q -O - "https://api.github.com/repos/InfiRandia/skyscraper/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/'`
	...
	wget -N https://github.com/InfiRandia/skyscraper/archive/${LATEST}.tar.gz || handle_error "fetch"
	echo "--- Unpacking ---"
